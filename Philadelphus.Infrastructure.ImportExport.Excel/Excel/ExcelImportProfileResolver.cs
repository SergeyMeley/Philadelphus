...
namespace Philadelphus.Infrastructure.ImportExport.Excel
{
    public class ExcelImportProfileResolver : IExcelImportProfileResolver
    {
        ...
        public ExcelImportProfile Resolve(string filePath, ExcelImportSourceSelection selection, ExcelImportProfile detectedProfile)
        {
            ...
            ApplyEntityKind(resolvedProfile, workbookDefault);
            ApplyEntityKind(resolvedProfile, worksheetDefault);
        ...
        private static ExcelImportProfile CloneProfile(ExcelImportProfile source)
        {
            return new ExcelImportProfile
            {
                ...
                DataStartRowOffset = source.DataStartRowOffset,
                Columns = source.Columns.Select(column => new ExcelImportColumnProfile
                {
                    ...
                    DataStartRowOffset = column.DataStartRowOffset
                }).ToList(),
        ...
        private static void ApplyEntityKind(ExcelImportProfile profile, ExcelImportSettingsRowDto? settingsRow)
        {
            if (settingsRow?.EntityKind.HasValue == true)
            {
                profile.EntityKind = settingsRow.EntityKind.Value;
            }
        }
        ...
        private static void ApplyRow(ExcelImportColumnProfile column, ExcelImportSettingsRowDto? settingsRow)
        {
            ...
            if (settingsRow.Placement.HasValue)
                column.Placement = settingsRow.Placement.Value;
            else if (settingsRow.DefinitionScope.HasValue)
                column.DefinitionScope = settingsRow.DefinitionScope.Value;

            if (settingsRow.PropagationMode.HasValue)
                column.PropagationMode = settingsRow.PropagationMode.Value;
            else if (settingsRow.ValueMode.HasValue)
