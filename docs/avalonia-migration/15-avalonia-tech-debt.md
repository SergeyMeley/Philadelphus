...
`ExcelImportDesignerWindow`: вкладки Листы/Связи/Предпросмотр перенесены. Вкладка **«Диаграмма»** (WPF `DiagramBehavior`, ~854 строки: `Canvas` со связями листов, перетаскивание узлов, зум) — **заглушка**. Перенос требует реализации диаграммы на Avalonia `Canvas` с жестами/зумом (крупная отдельная задача).
...
`Ribbon.Avalonia` выносит группы выбранной вкладки в свой контент, поэтому `DataContext`, заданный на `RibbonTab`, **до групп не наследуется** — биндинги внутри `RibbonGroupBox` резолвятся против `DataContext` ленты. Рабочий приём: задавать `DataContext` на самой `RibbonGroupBox` (как во вкладке «Формулы»→«Пересчет» и теперь «Вид»→«Тема»). **Открытый баг:** вкладка «Расширения» всё ещё задаёт `DataContext="{Binding ExtensionsControlVM}"` на `RibbonTab` — при наполнении списка расширений он не раскроется. Сейчас не проявляется, т.к. библиотеки расширений устарели после рефакторинга и список пуст. Фикс: перенести `DataContext` на группы вкладки «Расширения».
