...
- Тема: приложение работает в `RequestedThemeVariant=Light`; тёмная тема явно не тестировалась (часть мягких акцентов/нейтральных тонов взята из `DynamicResource`, но не везде).
